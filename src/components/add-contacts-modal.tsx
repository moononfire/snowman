    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-xl shadow-xl w-full max-w-lg flex flex-col border border-border" style={{ maxHeight: "80vh" }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <h2 className="font-semibold text-foreground">Dodaj kontakty do listy</h2>
        ...
        <div className="px-6 pt-4 shrink-0">
          <div className="relative">
            ...
            <Input className="pl-9" placeholder="Szukaj kontaktów..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-3 space-y-1">
          {contacts.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6">
              {search ? "Brak wyników" : "Wszyscy kontakty są już na liście lub brak kontaktów"}
            </p>
          )}
          {contacts.map((c) => (
            <button
        ...
        <div className="flex justify-between items-center px-6 py-4 border-t border-border shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={add} disabled={!selected.size || saving}>
              {saving ? "Dodawanie..." : `Dodaj ${selected.size}`}
            </Button>
          </div>
        </div>
      </div>
